...
9. Включить защиту от сканирования и сетевых атак (ограничивает до 4-х подключений с одного IP-адреса в течение 10 секунд, при превышении этого лимита IP-адрес блокируется на 10 секунд, также отключается ответ на пинг)
***
### Настройка
1. Установить патч для обхода блокировки протокола OpenVPN (только для UDP соединений)
```sh
